  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (servicesSection && quoteSection) {
        ...
        // Check if we're in Block 2 (Services Section)
        if (servicesRect.top <= 100 && quoteRect.top > 100) {
          setIsScrolled(true);
        }
        // If we're in Block 3 (Quote Section) or Block 1, use the original navbar style
        else {
          setIsScrolled(false);
        }
      }
