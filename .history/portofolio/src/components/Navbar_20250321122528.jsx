...
const NavContainer = styled.nav`
  ...
  right: 0;
  z-index: 100;
  background: ${props => props.location.pathname === '/about' 
    ? 'linear-gradient(to bottom, #fff 80%, rgba(255, 255, 255, 0))'
    : 'linear-gradient(to bottom, ${props.theme.colors.primary} 80%, rgba(0, 0, 0, 0))'};
...
const Logo = styled(Link)`
  ...
  padding: 0.2em 0.4em;
  border-image: ${props => props.location.pathname === '/about'
...
const NavItem = styled.div`
  ...
  border: 2px solid transparent;
  border-image: ${props => props.location.pathname === '/about'
    ? 'linear-gradient(45deg, #000, #fff, #000) 1'
...
const NavLink = styled(Link)`
  ...
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  text-shadow: ${props => props.location.pathname === '/about'
  ...
  return (
    <NavContainer location={location}>
      ...
      <NavLinks isOpen={isOpen}>
        <NavItem location={location} className={location.pathname === '/' ? 'active' : ''}>
          <NavLink to="/" className={location.pathname === '/' ? 'active' : ''}>
            Home
          </NavLink>
        </NavItem>
        
        <NavItem location={location} className={location.pathname === '/about' ? 'active' : ''}>
          <NavLink to="/about" className={location.pathname === '/about' ? 'active' : ''}>
            About Me
          </NavLink>
        </NavItem>
        
        <NavItem location={location} className={location.pathname === '/projects' ? 'active' : ''}>
          <NavLink to="/projects" className={location.pathname === '/projects' ? 'active' : ''}>
            Projects
          </NavLink>
        </NavItem>
        
        <NavItem location={location} className={location.pathname === '/contact' ? 'active' : ''}>
