...
const Home = () => {
  ...
  // Intersection observers with adjusted threshold and margin
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (inViewServices) {
        controlsServices.start({
          opacity: 1,
          y: 0,
          transition: { duration: isScrollingUp ? 0.8 : 0.6, ease: "easeOut" }
        });
      } else {
        controlsServices.start({
          opacity: 0,
          y: isScrollingUp ? 50 : 30,
  ...
  }, [lastScrollY, inViewServices, controlsServices]);
